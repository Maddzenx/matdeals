...
export const CompactQuantityControls: React.FC<CompactQuantityControlsProps> = ({
  ...
  if (quantity === 0) {
    return (
      <button
        onClick={onAdd}
        className="text-white text-center text-sm font-bold bg-[#DB2C17] px-4 py-2 rounded-[100px] w-full transition-colors active:bg-[#c12715] touch-manipulation"
      >
        Add
      </button>
    );
  ...
  return (
    <div className="flex items-center h-10 text-center text-sm font-bold bg-neutral-100 px-1.5 py-0 rounded-[100px]">
      <button
        onClick={onDecrement}
